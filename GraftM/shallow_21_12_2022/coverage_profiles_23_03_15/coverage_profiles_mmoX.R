...
setwd("/user_data/kalinka/GraftM/GraftM_packages/coverage_profiles_23_03_15")

#Importing the position file 
hits_mmoX<-vroom("/user_data/kalinka/GraftM/GraftM_packages/coverage_profiles_23_03_15/Position_files/position_of_mmoX_hits.tsv", delim = "\t")
hits_mmoX <- hits_mmoX %>%
...
ggsave("/user_data/kalinka/GraftM/GraftM_packages/coverage_profiles_23_03_15/Coverage_plots/cov_mmoX.png",
       cov_tax,
       height = 15,
       width = 15)
...
mmoX_diff_bar<-ggplot(mmoX_diff, aes(x = Taxonomy, y = ratio_cov, color =Taxonomy)) +
  geom_bar(stat = "identity", fill="white", linewidth=1.5) +
  ylab("Ratio between Median and Maximum Average Coverage")+
  #xlab("")+
  ylim(0,300)+
  coord_flip()+
  facet_wrap(~mfd_hab1)+
...
ggsave("/user_data/kalinka/GraftM/GraftM_packages/coverage_profiles_23_03_15/Coverage_plots/ratio_cov_mmoX.png",
       mmoX_diff_bar,
       height = 13,
       width = 15)
