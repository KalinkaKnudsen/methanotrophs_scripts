...
graftm_mmoX=/user_data/kalinka/GraftM/shallow_21_12_2022/user_data/kalinka/GraftM/shallow_21_12_2022/mmoX/graftm
cd $WD
odir=$WD/Position_files

#ls $graftm_mmoX > $odir/samples.txt samples file from pmoA
...
echo "Elapsed time for extracting HMM: $hmm_elapsed_time seconds" >> $odir/time_mmoX.txt
...
echo "Elapsed time for extracting tax: $tax_elapsed_time seconds" >> $odir/time_mmoX.txt
...
echo "Elapsed time for plotting in R: $R_elapsed_time seconds" >> $odir/time_mmoX.txt

combined_elapsed_time=$((R_time - start_time))
echo "Elapsed time for the entire script: $combined_elapsed_time seconds" >> $odir/time_mmoX.txt
